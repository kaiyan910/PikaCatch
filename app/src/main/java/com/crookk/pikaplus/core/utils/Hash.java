...
public class Hash {
    ...
    public Hash(String salt, int minHashLength) {
        this(salt, minHashLength, DEFAULT_ALPHABET);
    }

    public Hash(String salt, int minHashLength, String alphabet) {
        ...
        seps = Hash.consistentShuffle(seps, this.salt);
        ...
        alphabet = Hash.consistentShuffle(alphabet, this.salt);
  ...
  /* Private methods */
    ...
    private String _encode(long... numbers) {
        ...
        for (int i = 0; i < numbers.length; i++) {
            ...
            alphabet = Hash.consistentShuffle(alphabet, buffer.substring(0, alphabet.length()));
            String last = Hash.hash(num, alphabet);
    ...
    private long[] _decode(String hash, String alphabet) {
        ...
        if (!hashBreakdown.isEmpty()) {
            ...
            for (String aHashArray : hashArray) {
                ...
                alphabet = Hash.consistentShuffle(alphabet, buffer.substring(0, alphabet.length()));
                ret.add(Hash.unhash(subHash, alphabet));
            }
        }
    ...
    public static String encode(String salt, long... numbers) {
